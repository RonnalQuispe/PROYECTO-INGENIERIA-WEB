// coreCartera.js — Motor de Inteligencia de Cartera (3 capas)
...
function calcularMorosidad(ventas) {
    ...
    const ultimo = lags[lags.length-1];
    const z = d===0 ? 0 : (ultimo-m)/d;
    return { indiceMorosidad:round(1/(1+Math.exp(-0.5*z))), lagPromedio:Math.round(m), ultimoLag:ultimo };
}

// CAPA 3: Proyección con ciclos repetidos
function proyectar(clientes, umbral) {
    ...
    for (const c of clientes) {
        if (!c.saldoPendiente || c.saldoPendiente <= 0) continue;
        if (!c.ultimaCompra || !c.periodoCompra)        continue;

        const periodo       = c.periodoCompra;
        const confBase      = Math.exp(-1.5 * c.indiceMorosidad);
        const diasDesde     = dias(c.ultimaCompra, hoy);

        // Días hasta el PRÓXIMO ciclo usando módulo
        // diasDesde % periodo = cuántos días llevamos del ciclo actual
        // periodo - (diasDesde % periodo) = días que faltan para el siguiente
        const mod           = diasDesde % periodo;
        const offsetInicial = mod === 0 ? periodo : periodo - mod;

        const ciclosEnH  = Math.max(1, Math.floor(HORIZON / periodo));
        const montoCiclo = c.saldoPendiente / ciclosEnH;

        // DEBUG — quitar cuando funcione
        console.log(`[CORE] ${c.nombre}: periodo=${periodo} diasDesde=${diasDesde} offset=${offsetInicial} monto/ciclo=${montoCiclo.toFixed(2)} saldo=${c.saldoPendiente}`);

        let offset = offsetInicial;
        for (let k=0; offset <= HORIZON && k < 100; k++, offset += periodo) {
...
// ORQUESTADOR
function analizarCartera(ventas, umbral) {
    if (umbral === undefined) umbral = 200;
    const grupos = {};
    ventas.forEach(v => { if (!grupos[v.cliente]) grupos[v.cliente]=[]; grupos[v.cliente].push(v); });

    const clientes = Object.keys(grupos).map(nombre => {
        ...
        const ultima = new Date(Math.max.apply(null, vs.map(v=>new Date(v.fecha)))).toISOString();
        return {
            nombre,
    ...
    ventas.forEach(v => { if (v.cobros) v.cobros.forEach(c => { calor[new Date(c.fecha).getDay()] += (c.monto||0); }); });
